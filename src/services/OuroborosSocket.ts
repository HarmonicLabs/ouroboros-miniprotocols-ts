...
export class OuroborosSocket extends ServiceMap.Service<OuroborosSocket, {
   ...
   */
  getSocket: (
    protocolType: "node-to-node" | "node-to-client",
  ...
  static readonly layer = Layer.effect(
    OuroborosSocket,
    Effect.gen(function* () {
      const getSocket = Effect.fn("OuroborosSocket.getSocket")(
        function* (_protocolType: "node-to-node" | "node-to-client") {
          ...
          return yield* Effect.fail(
            new Socket.SocketError({
              reason: new Socket.SocketOpenError({
                kind: "Unknown",
                cause: new Error(
                  "Ouroboros socket adapter not yet implemented",
                ),
              }),
            }),
          );
