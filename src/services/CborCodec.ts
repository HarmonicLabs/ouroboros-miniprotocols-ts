...
export class CborCodec extends ServiceMap.Service<CborCodec, {
     ...
     */
    encodeValid: <S extends Schema.Top>(
        obj: unknown,
        schema: S,
    ) => Effect.Effect<
        Uint8Array,
        CborCodecError | Schema.SchemaError,
        S["DecodingServices"]
    >;
     ...
     */
    decodeValid: <S extends Schema.Top>(
        bytes: Uint8Array,
        schema: S,
    ) => Effect.Effect<
        S["Type"],
    ...
    static readonly layer = Layer.sync(
        ...
        () =>
            CborCodec.of({
                ...
                decode: Effect.fn("CborCodec.decode")(
                    function* (bytes: Uint8Array) {
                        return yield* Effect.try({
                            try: () => Cbor.parse(bytes),
                            catch: (error) =>
                                new CborCodecError({ cause: error }),
                        });
                    },
                ),
                encodeValid: Effect.fn("CborCodec.encodeValid")(
                    function* <S extends Schema.Top>(
                        ...
                        return yield* Effect.try({
                            try: () =>
                                Cbor.encode(validated as CborObj).toBuffer(),
                            catch: (error) =>
                                new CborCodecError({ cause: error }),
                        });
                    },
                ),
                decodeValid: Effect.fn("CborCodec.decodeValid")(
                    function* <S extends Schema.Top>(
                        bytes: Uint8Array,
                        schema: S,
                    ) {
