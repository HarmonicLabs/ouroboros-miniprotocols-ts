...
it.live("Handshake: propose N2N version and receive accept", () =>
  Effect.gen(function* () {
...
it.live("KeepAlive: send keepalive and receive response", () =>
  Effect.gen(function* () {
    // Handshake first
...
it.live("ChainSync: find intersect at origin and requestNext", () =>
  Effect.gen(function* () {
    // Handshake first
    const hs = yield* HandshakeClient;
    yield* hs.propose(preprodVersionTable);
...
it.live("BlockFetch: request block range from tip", () =>
  Effect.gen(function* () {
    // Handshake first
    const hs = yield* HandshakeClient;
    yield* hs.propose(preprodVersionTable);
