...
export class KeepAliveClient extends ServiceMap.Service<KeepAliveClient, {
  keepAlive: (cookie: number) => Effect.Effect<
    number,
    KeepAliveError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  done: () => Effect.Effect<
    void,
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.KeepAlive).pipe(
      ...
      const receiveOne = incoming.pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new KeepAliveError({ cause: "No response" })),
      ...
      return KeepAliveClient.of({
        keepAlive: Effect.fn("KeepAliveClient.keepAlive")(
          function* (cookie: number) {
            ...
            if (response._tag !== Schemas.KeepAliveMessageType.KeepAliveResponse) {
              return yield* Effect.fail(
                new KeepAliveError({ cause: `Unexpected message: ${response._tag}` }),
              );
            }
          ...
          function* () {
            yield* sendMessage({ _tag: Schemas.KeepAliveMessageType.Done });
          },
        ),
