  ...
  Schema.decodeTo(ChainSyncMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(ChainSyncMessage)(
        tuple[0] === 0 ? { _tag: ChainSyncMessageType.RequestNext }
          : tuple[0] === 1 ? { _tag: ChainSyncMessageType.AwaitReply }
          : tuple[0] === 2 ? { _tag: ChainSyncMessageType.RollForward, header: tuple[1], tip: tuple[2] }
          : tuple[0] === 3 ? { _tag: ChainSyncMessageType.RollBackward, point: tuple[1], tip: tuple[2] }
          : tuple[0] === 4 ? { _tag: ChainSyncMessageType.FindIntersect, points: tuple[1] }
          : tuple[0] === 5 ? { _tag: ChainSyncMessageType.IntersectFound, point: tuple[1], tip: tuple[2] }
          : tuple[0] === 6 ? { _tag: ChainSyncMessageType.IntersectNotFound, tip: tuple[1] }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case ChainSyncMessageType.RequestNext: return [0];
        case ChainSyncMessageType.AwaitReply: return [1];
        case ChainSyncMessageType.RollForward: return [2, msg.header, msg.tip];
        case ChainSyncMessageType.RollBackward: return [3, msg.point, msg.tip];
        case ChainSyncMessageType.FindIntersect: return [4, msg.points];
        case ChainSyncMessageType.IntersectFound: return [5, msg.point, msg.tip];
        case ChainSyncMessageType.IntersectNotFound: return [6, msg.tip];
        case ChainSyncMessageType.Done: return [7];
      }
    }),
  })
);
