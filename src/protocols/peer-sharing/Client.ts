...
export class PeerSharingError extends Schema.TaggedErrorClass<PeerSharingError>()("PeerSharingError", {
  cause: Schema.Defect,
}) {}
...
export class PeerSharingClient extends ServiceMap.Service<PeerSharingClient, {
  shareRequest: (amount: number) => Effect.Effect<
    ...
    PeerSharingError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  done: () => Effect.Effect<
    void,
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.PeerSharing).pipe(
      ...
      const receiveOne = incoming.pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new PeerSharingError({ cause: "No response" })),
      ...
      return PeerSharingClient.of({
        shareRequest: Effect.fn("PeerSharingClient.shareRequest")(
          function* (amount: number) {
            ...
            if (response._tag !== Schemas.PeerSharingMessageType.SharePeers) {
              return yield* Effect.fail(
                new PeerSharingError({ cause: `Unexpected message: ${response._tag}` }),
              );
            }
          ...
          function* () {
            yield* sendMessage({ _tag: Schemas.PeerSharingMessageType.Done });
          },
        ),
      });
    }),
