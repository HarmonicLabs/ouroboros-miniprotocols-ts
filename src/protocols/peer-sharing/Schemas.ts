...
const PeerAddressIPv4Cbor = Schema.Tuple([Schema.Literal(0), Schema.Uint8Array, Schema.Number]);
const PeerAddressIPv6Cbor = Schema.Tuple([Schema.Literal(1), Schema.Uint8Array, Schema.Number]);
  ...
  Schema.decodeTo(PeerAddressSchema, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(PeerAddressSchema)(
        tuple[0] === 0
          ? { _tag: PeerAddressType.IPv4, addr: tuple[1], port: tuple[2] }
          : { _tag: PeerAddressType.IPv6, addr: tuple[1], port: tuple[2] }
      ).pipe(
        Effect.mapError((_e) =>
...
const SharePeersCbor = Schema.Tuple([Schema.Literal(1), Schema.Array(PeerAddressFromCbor)]);
  ...
  Schema.decodeTo(PeerSharingMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(PeerSharingMessage)(
        tuple[0] === 0
          ? { _tag: PeerSharingMessageType.ShareRequest, amount: tuple[1] }
          : tuple[0] === 1
            ? { _tag: PeerSharingMessageType.SharePeers, peers: tuple[1] }
