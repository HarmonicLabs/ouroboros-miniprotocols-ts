...
const RequestRangeCbor = Schema.Tuple([Schema.Literal(0), ChainPointFromCbor, ChainPointFromCbor]);
  ...
  Schema.decodeTo(BlockFetchMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(BlockFetchMessage)(
        tuple[0] === 0 ? { _tag: BlockFetchMessageType.RequestRange, from: tuple[1], to: tuple[2] }
          : tuple[0] === 1 ? { _tag: BlockFetchMessageType.ClientDone }
          : tuple[0] === 2 ? { _tag: BlockFetchMessageType.StartBatch }
          : tuple[0] === 3 ? { _tag: BlockFetchMessageType.NoBlocks }
          : tuple[0] === 4 ? { _tag: BlockFetchMessageType.Block, block: tuple[1] }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case BlockFetchMessageType.RequestRange: return [0, msg.from, msg.to];
        case BlockFetchMessageType.ClientDone: return [1];
        case BlockFetchMessageType.StartBatch: return [2];
        case BlockFetchMessageType.NoBlocks: return [3];
        case BlockFetchMessageType.Block: return [4, msg.block];
        case BlockFetchMessageType.BatchDone: return [5];
      }
    }),
  })
);
