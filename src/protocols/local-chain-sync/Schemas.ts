...
import { ChainPointSchema, ChainPointFromCbor } from "../types/ChainPoint";
import { ChainTipSchema, ChainTipFromCbor } from "../types/ChainTip";
  ...
  Schema.decodeTo(LocalChainSyncMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(LocalChainSyncMessage)(
        tuple[0] === 0 ? { _tag: LocalChainSyncMessageType.RequestNext }
          : tuple[0] === 1 ? { _tag: LocalChainSyncMessageType.AwaitReply }
          : tuple[0] === 2 ? { _tag: LocalChainSyncMessageType.RollForward, block: tuple[1], tip: tuple[2] }
          : tuple[0] === 3 ? { _tag: LocalChainSyncMessageType.RollBackward, point: tuple[1], tip: tuple[2] }
          : tuple[0] === 4 ? { _tag: LocalChainSyncMessageType.FindIntersect, points: tuple[1] }
          : tuple[0] === 5 ? { _tag: LocalChainSyncMessageType.IntersectFound, point: tuple[1], tip: tuple[2] }
          : tuple[0] === 6 ? { _tag: LocalChainSyncMessageType.IntersectNotFound, tip: tuple[1] }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case LocalChainSyncMessageType.RequestNext: return [0];
        case LocalChainSyncMessageType.AwaitReply: return [1];
        case LocalChainSyncMessageType.RollForward: return [2, msg.block, msg.tip];
        case LocalChainSyncMessageType.RollBackward: return [3, msg.point, msg.tip];
        case LocalChainSyncMessageType.FindIntersect: return [4, msg.points];
        case LocalChainSyncMessageType.IntersectFound: return [5, msg.point, msg.tip];
        case LocalChainSyncMessageType.IntersectNotFound: return [6, msg.tip];
        case LocalChainSyncMessageType.Done: return [7];
      }
    }),
  })
);

export const LocalChainSyncMessageBytes = CborBytes(LocalChainSyncMessageFromCbor);
