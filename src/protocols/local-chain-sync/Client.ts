...
export class LocalChainSyncError extends Schema.TaggedErrorClass<LocalChainSyncError>()("LocalChainSyncError", {
  cause: Schema.Defect,
}) {}
...
export class LocalChainSyncClient extends ServiceMap.Service<LocalChainSyncClient, {
  requestNext: () => Effect.Effect<
    LocalChainSyncRollForward | LocalChainSyncRollBackward,
    LocalChainSyncError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.LocalChainSync).pipe(
            ...
            yield* incoming.pipe(
              ...
              Stream.runForEach((msg) => {
                ...
                return Deferred.fail(
                  response,
                  new LocalChainSyncError({ cause: `Unexpected message: ${msg._tag}` }),
                );
              }),
              Effect.forkChild,
            );

            yield* sendMessage({ _tag: Schemas.LocalChainSyncMessageType.RequestNext });
        ...
        findIntersect: Effect.fn("LocalChainSyncClient.findIntersect")(
          function* (points: ReadonlyArray<ChainPoint>) {
            ...
            return yield* incoming.pipe(
              ...
              Effect.flatMap(
                Option.match({
                  onNone: () => Effect.fail(new LocalChainSyncError({ cause: "No response" })),
          ...
          function* () {
            yield* sendMessage({ _tag: Schemas.LocalChainSyncMessageType.Done });
          },
        ),
      });
    }),
