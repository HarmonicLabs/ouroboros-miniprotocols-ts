...
export class LocalTxMonitorError extends Schema.TaggedErrorClass<LocalTxMonitorError>()("LocalTxMonitorError", {
  cause: Schema.Defect,
}) {}
...
export class LocalTxMonitorClient extends ServiceMap.Service<LocalTxMonitorClient, {
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.LocalTxMonitor).pipe(
      ...
      const receiveOne = incoming.pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new LocalTxMonitorError({ cause: "No response" })),
      ...
      const guardState = (expected: MonitorState) =>
        Ref.get(state).pipe(
          Effect.flatMap((current) =>
            current !== expected
              ? Effect.fail(new LocalTxMonitorError({ cause: `Invalid state: expected ${expected}, got ${current}` }))
              : Effect.void
          ),
        );
      ...
      return LocalTxMonitorClient.of({
        acquire: Effect.fn("LocalTxMonitorClient.acquire")(
          function* () {
            ...
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.Acquire });
            ...
            return yield* Effect.fail(
              new LocalTxMonitorError({ cause: `Unexpected message: ${response._tag}` }),
            );
          },
        ),
        nextTx: Effect.fn("LocalTxMonitorClient.nextTx")(
          function* () {
            ...
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.NextTx });
        ...
        hasTx: Effect.fn("LocalTxMonitorClient.hasTx")(
          function* (txId: Uint8Array) {
            yield* guardState("Acquired");
            yield* Ref.set(state, "Busy");
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.HasTx, txId });
        ...
        getSizes: Effect.fn("LocalTxMonitorClient.getSizes")(
          function* () {
            yield* guardState("Acquired");
            yield* Ref.set(state, "Busy");
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.GetSizes });
        ...
        release: Effect.fn("LocalTxMonitorClient.release")(
          function* () {
            yield* guardState("Acquired");
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.Release });
          ...
          function* () {
            yield* guardState("Idle");
            yield* sendMessage({ _tag: Schemas.LocalTxMonitorMessageType.Done });
