...
const ReplyGetSizesCbor = Schema.Tuple([Schema.Literal(8), Schema.Number, Schema.Number, Schema.Number]);
  ...
  Schema.decodeTo(LocalTxMonitorMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(LocalTxMonitorMessage)(
        tuple[0] === 0 ? { _tag: LocalTxMonitorMessageType.Acquire }
          : tuple[0] === 1 ? { _tag: LocalTxMonitorMessageType.Acquired, slot: tuple[1] }
          : tuple[0] === 2 ? { _tag: LocalTxMonitorMessageType.Release }
          : tuple[0] === 3 ? { _tag: LocalTxMonitorMessageType.NextTx }
          : tuple[0] === 4 ? { _tag: LocalTxMonitorMessageType.ReplyNextTx, tx: tuple[1] }
          : tuple[0] === 5 ? { _tag: LocalTxMonitorMessageType.HasTx, txId: tuple[1] }
          : tuple[0] === 6 ? { _tag: LocalTxMonitorMessageType.ReplyHasTx, hasTx: tuple[1] }
          : tuple[0] === 7 ? { _tag: LocalTxMonitorMessageType.GetSizes }
          : tuple[0] === 8 ? { _tag: LocalTxMonitorMessageType.ReplyGetSizes, sizes: { capacity: tuple[1], size: tuple[2], txCount: tuple[3] } }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case LocalTxMonitorMessageType.Acquire: return [0];
        case LocalTxMonitorMessageType.Acquired: return [1, msg.slot];
        case LocalTxMonitorMessageType.Release: return [2];
        case LocalTxMonitorMessageType.NextTx: return [3];
        case LocalTxMonitorMessageType.ReplyNextTx: return [4, msg.tx];
        case LocalTxMonitorMessageType.HasTx: return [5, msg.txId];
        case LocalTxMonitorMessageType.ReplyHasTx: return [6, msg.hasTx];
        case LocalTxMonitorMessageType.GetSizes: return [7];
        case LocalTxMonitorMessageType.ReplyGetSizes: return [8, msg.sizes.capacity, msg.sizes.size, msg.sizes.txCount];
        case LocalTxMonitorMessageType.Done: return [9];
      }
    }),
  })
);
