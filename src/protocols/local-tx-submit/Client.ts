...
export class LocalTxSubmitError extends Schema.TaggedErrorClass<LocalTxSubmitError>()("LocalTxSubmitError", {
  cause: Schema.Defect,
}) {}
...
export class LocalTxSubmitClient extends ServiceMap.Service<LocalTxSubmitClient, {
  submit: (tx: Uint8Array) => Effect.Effect<
    Schemas.LocalTxSubmitResult,
    LocalTxSubmitError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  done: () => Effect.Effect<
    void,
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.LocalTxSubmission).pipe(
      ...
      const receiveOne = incoming.pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new LocalTxSubmitError({ cause: "No response" })),
        ...
        submit: Effect.fn("LocalTxSubmitClient.submit")(
          function* (tx: Uint8Array) {
            ...
            if (response._tag === Schemas.LocalTxSubmitMessageType.RejectTx) {
              return { accepted: false as const, reason: response.reason };
            }

            return yield* Effect.fail(
              new LocalTxSubmitError({ cause: `Unexpected message: ${response._tag}` }),
            );
          },
        ),
        done: Effect.fn("LocalTxSubmitClient.done")(
          function* () {
            yield* sendMessage({ _tag: Schemas.LocalTxSubmitMessageType.Done });
          },
        ),
      });
    }),
