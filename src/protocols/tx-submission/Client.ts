...
export class TxSubmissionError extends Schema.TaggedErrorClass<TxSubmissionError>()("TxSubmissionError", {
  cause: Schema.Defect,
}) {}

export interface TxSubmissionHandlers {
  readonly onRequestTxIds: (ack: number, req: number, blocking: boolean) => Effect.Effect<
...
export class TxSubmissionClient extends ServiceMap.Service<TxSubmissionClient, {
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.TxSubmission).pipe(
      ...
      return TxSubmissionClient.of({
        run: Effect.fn("TxSubmissionClient.run")(
          function* (handlers: TxSubmissionHandlers) {
            ...
            yield* sendMessage({ _tag: Schemas.TxSubmissionMessageType.Init });
              ...
              Stream.runForEach((msg) =>
                Effect.gen(function* () {
                  if (msg._tag === Schemas.TxSubmissionMessageType.RequestTxIds) {
                    ...
                    yield* Effect.fail(
                      new TxSubmissionError({ cause: `Unexpected server message: ${msg._tag}` }),
                    );
                  }
                })
              ),
          ...
          function* () {
            yield* sendMessage({ _tag: Schemas.TxSubmissionMessageType.Done });
          },
        ),
      });
    }),
