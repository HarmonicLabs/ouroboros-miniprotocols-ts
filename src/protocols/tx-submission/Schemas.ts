...
const RequestTxIdsCbor = Schema.Tuple([Schema.Literal(0), Schema.Boolean, Schema.Number, Schema.Number]);
const ReplyTxIdsCbor = Schema.Tuple([Schema.Literal(1), Schema.Array(TxIdAndSizeCbor)]);
const RequestTxsCbor = Schema.Tuple([Schema.Literal(2), Schema.Array(Schema.Uint8Array)]);
const ReplyTxsCbor = Schema.Tuple([Schema.Literal(3), Schema.Array(Schema.Uint8Array)]);
  ...
  Schema.decodeTo(TxSubmissionMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(TxSubmissionMessage)(
        tuple[0] === 0 ? { _tag: TxSubmissionMessageType.RequestTxIds, blocking: tuple[1], ack: tuple[2], req: tuple[3] }
          : tuple[0] === 1 ? { _tag: TxSubmissionMessageType.ReplyTxIds, ids: (tuple[1] as ReadonlyArray<readonly [Uint8Array, number]>).map(([txId, size]) => ({ txId, size })) }
          : tuple[0] === 2 ? { _tag: TxSubmissionMessageType.RequestTxs, txIds: tuple[1] }
          : tuple[0] === 3 ? { _tag: TxSubmissionMessageType.ReplyTxs, txs: tuple[1] }
          : tuple[0] === 4 ? { _tag: TxSubmissionMessageType.Done }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case TxSubmissionMessageType.RequestTxIds: return [0, msg.blocking, msg.ack, msg.req];
        case TxSubmissionMessageType.ReplyTxIds: return [1, msg.ids.map((id): readonly [Uint8Array, number] => [id.txId, id.size])];
        case TxSubmissionMessageType.RequestTxs: return [2, msg.txIds];
        case TxSubmissionMessageType.ReplyTxs: return [3, msg.txs];
        case TxSubmissionMessageType.Done: return [4];
        case TxSubmissionMessageType.Init: return [6];
      }
    }),
  })
);
