  ...
  Schema.decodeTo(LocalStateQueryMessage, {
    decode: SchemaGetter.transformOrFail((tuple) =>
      Schema.decodeUnknownEffect(LocalStateQueryMessage)(
        tuple[0] === 0 ? { _tag: LocalStateQueryMessageType.Acquire, point: tuple[1] }
          : tuple[0] === 1 ? { _tag: LocalStateQueryMessageType.Acquired }
          : tuple[0] === 2 ? { _tag: LocalStateQueryMessageType.Failure, failure: tuple[1] }
          : tuple[0] === 3 ? { _tag: LocalStateQueryMessageType.Query, query: tuple[1] }
          : tuple[0] === 4 ? { _tag: LocalStateQueryMessageType.Result, result: tuple[1] }
          : tuple[0] === 5 ? { _tag: LocalStateQueryMessageType.ReAcquire, point: tuple[1] }
          : tuple[0] === 6 ? { _tag: LocalStateQueryMessageType.Release }
    ...
    encode: SchemaGetter.transform((msg) => {
      switch (msg._tag) {
        case LocalStateQueryMessageType.Acquire: return [0, msg.point];
        case LocalStateQueryMessageType.Acquired: return [1];
        case LocalStateQueryMessageType.Failure: return [2, msg.failure];
        case LocalStateQueryMessageType.Query: return [3, msg.query];
        case LocalStateQueryMessageType.Result: return [4, msg.result];
        case LocalStateQueryMessageType.ReAcquire: return [5, msg.point];
        case LocalStateQueryMessageType.Release: return [6];
        case LocalStateQueryMessageType.Done: return [7];
      }
    }),
  })
);
