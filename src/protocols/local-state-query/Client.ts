...
export class LocalStateQueryError extends Schema.TaggedErrorClass<LocalStateQueryError>()("LocalStateQueryError", {
  cause: Schema.Defect,
}) {}
...
export class LocalStateQueryClient extends ServiceMap.Service<LocalStateQueryClient, {
  acquire: (point?: ChainPoint) => Effect.Effect<
    void,
    AcquireFailure | LocalStateQueryError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  query: (query: Uint8Array) => Effect.Effect<
    Uint8Array,
    LocalStateQueryError | MultiplexerEncodingError | Socket.SocketError | Schema.SchemaError | Cause.TimeoutError,
    Scope.Scope
  >;
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer.getProtocolChannel(MiniProtocol.LocalStateQuery).pipe(
      ...
      const receiveOne = incoming.pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () => Effect.fail(new LocalStateQueryError({ cause: "No response" })),
      ...
      const guardState = (expected: QueryState) =>
        Ref.get(state).pipe(
          Effect.flatMap((current) =>
            current !== expected
              ? Effect.fail(new LocalStateQueryError({ cause: `Invalid state: expected ${expected}, got ${current}` }))
              : Effect.void
          ),
        );
      ...
      return LocalStateQueryClient.of({
        acquire: Effect.fn("LocalStateQueryClient.acquire")(
          function* (point?: ChainPoint) {
            ...
            if (response._tag === Schemas.LocalStateQueryMessageType.Failure) {
              ...
              return yield* Effect.fail(new AcquireFailure({ failure: response.failure }));
            }

            return yield* Effect.fail(
              new LocalStateQueryError({ cause: `Unexpected message: ${response._tag}` }),
            );
          },
        ),
        ...
        release: Effect.fn("LocalStateQueryClient.release")(
          function* () {
            yield* guardState("Acquired");
            yield* sendMessage({ _tag: Schemas.LocalStateQueryMessageType.Release });
            yield* Ref.set(state, "Idle");
          },
        ),
          ...
          function* () {
            yield* guardState("Idle");
            yield* sendMessage({ _tag: Schemas.LocalStateQueryMessageType.Done });
